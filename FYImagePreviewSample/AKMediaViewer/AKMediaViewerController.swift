...
public class AKMediaViewerController : UIViewController, UIScrollViewDelegate {
    ...
    private var originalTitleLabelFrame:CGRect?
    
    @IBAction func buttonActionShowMorePressed(sender: UIButton) {
        
        sender.selected = !sender.selected
        
        if sender.selected {
            originalTitleLabelFrame = self.titleLabel.frame
            self.titleLabel.numberOfLines = 0
            self.titleLabel.sizeToFit()
            self.titleLabel.frame = CGRectOffset(self.titleLabel.frame, 0, -self.titleLabel.frame.height+30)
        }else {
            self.titleLabel.frame = originalTitleLabelFrame!
